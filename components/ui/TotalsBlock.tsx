      ...
      <dl className="flex flex-col gap-2.5">
        {rows.map((row) => (
          <div key={row.label} className="flex items-baseline justify-between gap-4">
            ...
            <dd className="text-row text-ink-secondary tabular-nums">{row.value}</dd>
          </div>
        ))}
        <div className="mt-1 flex items-baseline justify-between gap-4 border-t border-hairline pt-3">
          ...
          <dd className="text-price text-ink tabular-nums">{total.value}</dd>
        </div>
      </dl>
      <p className="mt-3 text-legal text-ink-faint">{footnote}</p>
    </div>
  );
}
